...
public class NationalPlayerTest {
	...
	public void beforeTest() {
		...
		// Player from India
		indiaPlayer = new NationalPlayer().withCountry(Country.INDIA).withCareerGoals(100).withFirstName("Sanjay")
				.withLastName("Acharya").withWorldRanking(1);

		// Player from Usa
		usaPlayer = new NationalPlayer().withCountry(Country.USA).withCareerGoals(80).withFirstName("Blake")
				.withLastName("Acharya").withWorldRanking(10);

		italyPlayer = new NationalPlayer().withCountry(Country.ITALY).withCareerGoals(20).withFirstName("Valentino")
				.withLastName("Acharya").withWorldRanking(32);
	}
	...
	public void testSimpleCriteira() throws Exception {
		...
		try {
			Criteria c = session.createCriteria(NationalPlayer.class).add(Restrictions.eq("country", Country.INDIA));
			...
			c = session.createCriteria(NationalPlayer.class).add(Restrictions.between("worldRanking", 5, 15));
			players = c.list();
			assertEquals("Should only have the usa player", 1, players.size());
			assertContainsPlayers(players, usaPlayer);

			c = session.createCriteria(NationalPlayer.class).add(Restrictions.eq("lastName", "Acharya"));
			players = c.list();
	...
	/**
	 * Simple Method that ensures that the list contains all the players
	 * provided.
	 * 
	 * @param playerList
	 *            List of Players
	 ...
	 *            Display message
	 * @param shardId
	 *            Expected to exist in this shard only
	 * @param playerId
	 *            Player identifier
	 */
